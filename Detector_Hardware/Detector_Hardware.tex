...
The correction factor of 0.5 is added in order to make the assumption that the boards are constructed homogeneously down its length. For this we need to take the decreasing length of each channel transmission line into account.
...
In addition the the transmission lines the \railboard\ is equipped with ground planes.
...
One layer consists of 34.3 transmission lines and 39.3 guard lines each \SI{0.4}{mm} wide with a \SI{0.3}{mm} gap in between.
This results in a line structure with the width of $W_\text{total}$.
\begin{align}
	W_\text{total} 	&= \left(34.3 + 39.3\right) \cdot \SI{0.4}{mm} + 2 \cdot 34.3 \cdot \SI{0.3}{mm} \\
					&= \SI{50.02}{mm}
\end{align}
...
\[T_{\text{eff ground}} = \frac{1}{2} \cdot \SI{50.02}{mm} \cdot 7 \cdot \SI{17}{\micro m} \cdot \frac{1}{\SI{185}{mm}} = \SI{16}{\micro m}\]
This gives us a total copper effective thickness of $\SI{9.3}{\micro m} + \SI{16}{\micro m} = \SI{25.3}{\micro m}$.
