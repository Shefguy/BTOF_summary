...
\begin{table*}[htbp]
	\caption[Time resolution for different wrapping materials]{Time resolution of EJ-232 (top) plastic scintillator tiles for various wrapping materials.
...
This offers a larger active area without increasing the detector complexity by adding unnecessary channels when reading out the \sipms\ individually.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[height=5cm]{fig/SensorBoardNew3.png}
...
A possible layout to achieve a characteristic impedance of \SI{50}{\ohm} would be a flex board from \texttt{beta-layout.com} who produce flexible embedded microstrip boards with a substrate thickness of \SI{80}{\micro m} with \SI{50}{\micro m} of cover film on top, a copper strength of \SI{18}{\micro m} and a dielectric constant of 3.2 to 3.3 for a transmission line width of \SI{257}{\micro m}.
%With a line width of \SI{257}{\micro m} this produces an impedance of \SI{49.94}{\ohm}.

Due to an error while calculating the expected impedance the second board version has increased transmission line widths of \SI{0.4}{mm} resulting in an impedance of \SI{38.4}{\ohm}.

%Our first version of these boards however were not properly impedance matched since they had.\todo[]{what are the dimensions of the first boards?}

One important finding during the testing of this new board is that the thin copper lines are very fragile.
...
Another issue with these \sensorboard s is that when bent in certain ways the boards produce distortions, including reflections, in the signal.
One possible explanation is the fact that by changing the geometry of the board out of a two dimensional plane by bending it, the expected characteristic impedance changes and no longer is close to \SI{50}{\ohm}.
Since this effect was observed in varying strength depending on the probed channel the layout of the specific \railboard\ channel might contribute in unknown ways.
...
Ultimately this development would need to be made in combination with finalizing the electronics of the whole system and still requires some effort to get right.
The key is to produce a strong pulse ideally without interfering with the \sipm\ readout.
...
\begin{figure}[htbp]
	\centering
	\includegraphics*[width=1.\textwidth]{fig/Railboard3_imageCombined.png}
	\caption{Combined depiction of the internal transmission line layout and a photograph of the front part of the \railboard .}
	\label{fig:Railboard}
\end{figure}

Each board consists of 16 conductor layers alternating between shielding ground layers, which span almost the entire layer and signal transmission lines, which can be seen in \fig ~\ref{fig:Railboard3_schematic}.
...
This resultes in an actual board thickness of \SI{3.4}{mm}.
...
It needs to be evaluated if the gains from using Rogers Material outweigh the drawbacks of splitting the boards.
...
\subsection{Carbon Fiber Frame}
In order to provide a structural skeleton for the detector and shield the photo sensitive sensors from external light sources the \sm s are all positioned inside a carbon fiber box depicted in \fig~\ref{fig:Carbon_frame}.
This box also provides simple accessibility to the detector components since they can be slid in and out of the slot dedicated to the detector module.

To achieve this the frame should wrap around the entire detector while using minimal material.
A primitive layout is available which uses three \SI{1}{mm} thick carbon sheets for the main sides.
The lid consist of a single \SI{1}{mm} thick piece, while an additional slotted sheet is added to the bottom.
The sides of this layout consist of long L-pieces.

The frame also consist of small 3-D printed spacer which not only divide the space into slots for the scintillators, but also provide screw holes to fasten the lid and the \railboard s to the base.
The scintillators are simply held in place by loosely constraining their movement in every direction.
Since the scintillators do not sit in the slots too tightly, the small leeway allows for slight moving and bending of the \sm\ without applying to much pressure to individual components.

\begin{figure}
	\centering
	\includegraphics[height=.9\textwidth, angle=270]{fig/Grundplatte-Baugruppe.pdf}
	\caption{Drawing of the carbon fiber frame with slots to save mass and small spacer to position the scintillator tiles.}
	\label{fig:Carbon_frame}
\end{figure}
...
Since 60 channels need to be connected and each channel needs a guard trace to minimize crosstalk two cables are necessary, otherwise the pitch of the connectors would get too small.
The design of these cables has not been studied or optimized.
They should ideally possess a characteristic impedance of \SI{50}{\ohm}.
The cables used for the prototype do not quite fulfill that requirement.
Each transmission line meant for \sipm\ signals is \SI{0.3}{mm} wide with a separation of \SI{0.2}{mm} between lines.
...
This however limits the width of each line in order to fit them all in the \SI{32.5}{mm} wide cable.
...
\begin{figure}[htbp]
	...
	\caption{Connector for power and LED signals between \railboard s, which sit behind the signal connectors, just ouside of the scope of \fig~\ref{fig:Railboard_connection}.}
...
In order to affect the calorimeter performance as little as possible the mass of the \btof\ has to be minimized.
...
Since the density ($\rho$) of the Rogers Material is known the other densities can be estimated. The density of an object is the sum of the density of its components weighted by the fraction of the volume it occupies as described in Equation~\eqref{eq:RogersDensity}.
\begin{align}
...
With a substrate thickness of \SI{0.2}{mm} per layer and 15 layers of substrate this amounts to a thickness of $\SI{0.2}{mm} \times 15 = \SI{3}{mm}$ which is slightly below the measured thickness of \SI{3.4}{mm}.
...
Studies on the radiation hardness were not extended beyond what is available in the TDR.
It was planned and attempted to measure the expected damage to the \sipms\ ourselves by joining the regular irradiation tests performed by the EMC group in Gießen.
Goal of this analysis was to compare the performance of \sipms\ before and after irradiation.
In particular the leakage current and dark count rate which should increase due to damage to the silicon lattice.
However repeated delays due to issues at the irradiation facility in Groningen lead to this analysis also being delayed.
Due to personal and time limitations the analysis was also never completed.

The scintillators however were studied and show little damage.
A plot of the spectral transmittance is shown in \fig~\ref{fig:scintillator_radiation_losses} plotting the transmittance spectra for wavelengths from \SIrange{200}{900}{nm}.
Slight losses especially at lower wavelengths are visible, but nothing which would diminish the detectors performance or be a cause of concern for the detector.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=.8\textwidth]{fig/Scintillator_radiation_losses.png}
	\caption{Transmittance of EJ-232 scintillator tiles at various stages of irradiation.}
	\label{fig:scintillator_radiation_losses}
\end{figure}
