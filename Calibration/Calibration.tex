...
Measuring the time resolution at each slot along the entire \railboard\ gives fairly similar good results in the 90 to 160 ps range (see Fig. ~\ref{fig:TR}), but the time difference between the two signals increases on the second part (see Fig. ~\ref{fig:Diff}, shown for the left part of the board) of the railboard due to waveform distortion (reflections, different signal attenuation on two sides of one slot).
\\
\begin{figure}[h!]
\includegraphics[scale=0.5]{Pictures/TimeRes.pdf}
\\
\textbf{Figure ~\ref{fig:TR}: blabla.}
\label{fig:TR}
\end{figure}
\\
